...
export const handler = async (
  ...
  const res: SVGData = {
    ...
    paths: [
      `M 0 0 L 0 ${data.size[1]} L ${data.size[0]} ${data.size[1]} L ${
        data.size[0]
      } 0 L 0 0 M ${data.x[0]} ${data.size[1]}`,
    ],
  };
  let i = 0;
  while (i < data.x.length) {
    if (body.type === "scatter") {
      res.paths[0] += ` M ${data.x[i]} ${
        data.size[1] - data.y[i]
      } l 0.5 0.5 l -1 -1 l 0.5 0.5 l 0.5 -0.5 l -1 1`;
  ...
  const response = {
    ...res,
    svg:
      `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="${res.viewbox}" fill="white" stroke="black" stroke-width="0.5">${
        res.paths.map((x) => `<path d = "${x}" />`).join("")
      }</svg>`,
  };
  return body.res === "svg"
    ? new Response(response.svg)
    : new Response(JSON.stringify(response));
};

function validateBody(
  body: Record<string, unknown>,
): { size: [number, number]; x: number[]; y: number[] } | Response {
  if (body.sent === "pairs") {
    console.log(body);
    if (
    ...
    if (body.points.length > 5000) {
      return new Response(null, {
        status: 400,
        statusText: "Cannot visualize more than 5000 points.",
      });
    }
    const size = [
      Math.max(
    ...
    if (
      !body.x || !Array.isArray(body.x) ||
      body.x.some((x) => !isFinite(Number(x)))
    ) {
      return new Response(null, {
        status: 400,
        statusText:
    ...
    if (
      !body.y || !Array.isArray(body.y) || body.y.some((x) =>
        !isFinite(Number(x))
      ) ||
